...
export async function searchRows(companyIds: string[], term: string, grants: SearchGrants): Promise<SearchRow[]> {
  ...
  const parts: SQL[] = [
    sql`(SELECT 'product' AS kind, i.id::text AS id, i.name AS title, i.sku AS subtitle
           FROM items i
          WHERE i.company_id IN (${companies}) AND (i.name ILIKE ${pattern} OR i.sku ILIKE ${pattern})
          ORDER BY i.name LIMIT ${PER_KIND})`,
    sql`(SELECT 'contact', c.id::text, c.display_name, coalesce(c.phone, c.company_name, c.city)
           FROM contacts c
    ...
    sql`(SELECT 'payment', d.id::text, d.number, coalesce(ct.display_name, '') || ' · ' || d.grand_total
           FROM documents d
           JOIN document_types dt ON dt.id = d.document_type_id
           LEFT JOIN contacts ct ON ct.id = d.contact_id
    ...
    // named by.
    sql`(SELECT 'expense', e.id::text, coalesce(nullif(e.notes, ''), ec.name), ec.name || ' · ' || e.amount
           FROM expenses e
    ...
    sql`(SELECT 'category', c.id::text, c.name, '' FROM categories c
          WHERE c.name ILIKE ${pattern} ORDER BY c.name LIMIT ${PER_KIND})`,
    sql`(SELECT 'brand', b.id::text, b.name, '' FROM brands b
          WHERE b.name ILIKE ${pattern} ORDER BY b.name LIMIT ${PER_KIND})`,
    sql`(SELECT 'unit', u.id::text, u.name, coalesce(u.symbol, '') FROM units u
          WHERE u.name ILIKE ${pattern} OR u.symbol ILIKE ${pattern} ORDER BY u.name LIMIT ${PER_KIND})`,
    sql`(SELECT 'location', l.id::text, l.name, l.location_type::text FROM locations l
          WHERE l.name ILIKE ${pattern} OR l.code ILIKE ${pattern} ORDER BY l.name LIMIT ${PER_KIND})`,
    sql`(SELECT 'tax', t.id::text, t.name, t.rate::text || '%' FROM taxes t
    ...
    sql`(SELECT 'company', co.id::text, co.name, coalesce(co.short_name, '') FROM companies co
  ...
  return db.execute<SearchRow>(sql.join(parts, sql` UNION ALL `));
}
