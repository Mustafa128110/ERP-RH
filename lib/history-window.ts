...
export type HistoryInfo = { page: number; pageSize: number; total: number; query: string; sort: string; direction: "asc" | "desc"; all: boolean };
export function historyRequest(params: Record<string, string | string[] | undefined>): HistoryRequest {
  ...
  return {page:Math.max(1,Math.floor(Number(value("page"))||1)),query:value("q")?.slice(0,300),sort:value("sort"),direction:value("direction"),all:value("all")==="1"};
}
