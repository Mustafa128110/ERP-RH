...
const bytes = (inv: Invoice) => (buildInvoicePdf(inv).output("arraybuffer") as ArrayBuffer).byteLength;

// A full invoice produces a real file, and it is a PDF.
const doc = buildInvoicePdf(base);
...
assert.ok(bytes({ ...base, lines: [] }) > 1000, "an invoice with no lines should still render");

// The adjustment rows are conditional — exercise the branch where all three show.
assert.ok(
  bytes({ ...base, discountTotal: "500", taxTotal: "170.5", shippingTotal: "300", paidAmount: "1000" }) > 1000,
...
assert.ok(buildInvoicePdf(many).getNumberOfPages() > 1, "80 lines should span more than one page");
...
const text = (inv: Invoice) => Buffer.from(buildInvoicePdf(inv).output("arraybuffer") as ArrayBuffer).toString("latin1");

const branded = text({ ...base, companyName: "M52 Trading", companyEmail: "mustafa@royalhardware.co" });
