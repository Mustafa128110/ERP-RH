-- Rebuild FIFO once more with the same settlement-account ownership guard used
-- by the application. Historical cross-company payments remain visible for
-- correction but cannot alter a different company's party balances.
