...
export function RecordManager<T extends { id: string }>({
  ...
  renderBatchDialog: (args: { onClose: () => void; onDone: () => void }) => ReactNode;
  ...
  return (
    <div className="flex h-full flex-col gap-2">
      ...
      <DataTable
        ...
        idKey="id"
        onRowClick={(row) => {
          const record = records.find((r) => r.id === String(row.id));
      ...
      {modal?.kind === "batch" && renderBatchDialog({ onClose: () => setModal(null), onDone: close })}
