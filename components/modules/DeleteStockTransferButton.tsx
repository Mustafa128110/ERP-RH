...
export function DeleteStockTransferButton({ transferId }: { transferId: string }) {
  ...
  // cancelled transfer's own page is what's on screen, so leaving it is part of
  // the action. A failure keeps you here with the error visible.
  ...
  return (
    <form
      action={action}
      onSubmit={(e) => {
        if (!confirm("Cancel this transfer? Its history will remain and the stock it moved will go back to where it was.")) e.preventDefault();
      }}
    >
