...
export function InvoiceImageRenderer({ invoice, onReady }: { invoice: Invoice; onReady: (node: HTMLElement) => void }) {
  ...
  return (
    ...
    <div ref={ref} aria-hidden className="force-light pointer-events-none fixed top-0 left-0 z-[-1000] w-[768px] bg-white">
      <InvoiceDocument invoice={invoice} companyName="Royal Hardware" />
    </div>
  );
}
...
export function InvoiceDocument({ invoice, companyName }: { invoice: Invoice; companyName?: string }) {
  ...
  const displayName = companyName ?? invoice.companyName;
  ...
  return (
    ...
    >
      ...
      <div className="scroll-thin overflow-x-auto print:overflow-visible">
        ...
        <thead>
          <tr className="border-y border-sand">
            <th className="w-10 py-2 text-center text-xs font-semibold uppercase tracking-wide text-steel">#</th>
      ...
      <div className="mt-6 flex justify-end">
        <dl className="grid w-72 grid-cols-[1fr_auto] gap-y-1 text-sm">
          ...
          {invoice.previousBalance > 0 && (
            <>
              <dt className="text-steel">Previous Balance</dt>
              <dd className="text-right tabular-nums">{money(invoice.previousBalance)}</dd>
            </>
          )}
          <dt className="mt-1 border-t border-sand pt-1 font-semibold text-ink">Grand Total</dt>
          <dd className="mt-1 border-t border-sand pt-1 text-right font-semibold tabular-nums text-ink">{money(Number(invoice.grandTotal) + invoice.previousBalance)}</dd>
      ...
      <p className="mt-10 border-t border-sand pt-4 text-xs text-steel">
        {invoice.invoiceFooter?.trim() || `Generated by ${displayName}. Goods once sold are not returnable without this invoice.`}
      </p>
    </div>
  );
}
