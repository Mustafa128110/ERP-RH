...
await build({entryPoints:['lib/history-actions.check.ts'],outfile:'db-export/history-actions.cjs',bundle:true,packages:'external',platform:'node',format:'cjs',plugins:[{name:'framework-revalidation',setup(builder){
...
const result=spawnSync(process.execPath,['--conditions=react-server','db-export/history-actions.cjs'],{stdio:'inherit',windowsHide:true,env:{...process.env,UPSTASH_REDIS_REST_URL:'',UPSTASH_REDIS_REST_TOKEN:''}});
